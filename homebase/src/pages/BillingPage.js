...
export default async function BillingPage(container) {
  ...
  const startCheckout = withBusy(async (tier) => {
    try {
      const res = await auth.createCheckout(tier);
      if (!res || !res.url) {
  ...
  const cancelPlan = withBusy(async () => {
    if (!window.confirm('cancel your thay-sub? you\'ll drop to thay free immediately (apps keep working, paid entitlements pause).')) return;
    try {
      await auth.cancelSubscription();
      toast('subscription canceled — back on thay free', 'info');
      location.hash = '#/billing';
      location.reload();
    } catch (err) {
  ...
  const currentPlan = h('div', { className: 'glass-card-static billing-current' }, [
    h('div', { className: 'section-header' }, [
      h('h3', {}, ['your thay-sub']),
      h('span', { className: 'input-hint' }, [`${plan.monthly === 0 ? 'free forever' : plan.monthly === -1 ? 'custom' : `$${plan.monthly}/mo`}`]),
    ]),
  ...
  const tierGrid = h('div', { className: 'billing-tier-grid' }, tierOptions.map((t) => {
    ...
    return h('div', { className: 'billing-tier' + (isCurrent ? ' billing-tier--current' : ''), style: isCurrent ? { borderColor: 'var(--vibe-accent)' } : {} }, [
      ...
      isCurrent
        ...
        : t.id === 'free'
          ? h('button', { className: 'btn btn-ghost btn-sm', onClick: cancelPlan }, ['downgrade'])
          : h('button', { className: 'btn btn-ghost btn-sm', onClick: () => startCheckout(t.id) }, ['choose']),
    ]);
  }));
  ...
  const purchaseRows = purchases.length
    ? purchases.map((p) => h('div', { className: 'device-item', style: { flexWrap: 'wrap' } }, [
        ...
        h('div', { style: { display: 'flex', gap: 'var(--space-sm)', alignItems: 'center' } }, [
          ...
          p.owned
            ? null
            : h('button', { className: 'btn btn-ghost btn-sm', onClick: () => startCheckout('pro') }, ['buy']),
        ]),
      ].filter(Boolean)))
